    ...
    return (
        <div className="w-full space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* 🟢 LEFT MODULE TRACK: APPLICATION SUBMISSION (col-md-4 equivalent) */}
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 h-fit">
                    ...
                    <form onSubmit={handleApplySubmit} className="space-y-4">
                        ...
                        <button
                            type="submit"
                            disabled={saving}
                            className="w-full bg-cyan-600 hover:bg-cyan-700 text-white font-semibold py-2.5 rounded-xl transition cursor-pointer text-sm"
                        >
                ...
                {/* 🟢 RIGHT MODULE TRACK: DATA VIEW LIST (col-md-8 equivalent) */}
                                                ...
                                                {item.status === "Pending" && isAdmin ? (
                                                    ...
                                                    <div className="flex flex-col gap-2 min-w-[160px]">
                                                        <input
                                                            ...
                                                            className="border border-slate-200 p-1 text-xs rounded-lg w-full focus:outline-none focus:ring-1 focus:ring-cyan-500"
                                                        />
                                                        <div className="flex gap-2 justify-end">
                                                            <button
                                                                onClick={() => handleAdminAction(item.id, "Approved")}
                                                                className="bg-green-600 hover:bg-green-700 text-white text-xs px-2.5 py-1 rounded-lg transition font-medium cursor-pointer"
                                                            >
                                                                Approve
                                                            </button>
                                                            <button
                                                                onClick={() => handleAdminAction(item.id, "Declined")}
                                                                className="bg-red-500 hover:bg-red-700 text-white text-xs px-2.5 py-1 rounded-lg transition font-medium cursor-pointer"
                                                            >
                                                                Decline
                                                            </button>
                                                        </div>
